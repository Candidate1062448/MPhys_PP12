...
def peak_threshold_finder301(y, width = 4):
    '''
    Input: y = signal array
           width = expected width of the peak (optional) (default =4)
    ...
    for i in crossing:
        for j in range(width*3):
            if np.all(y[i+j+1:i+j+width]<y[i+j])==True:
                positions.append(i+j)
                break
            #else:
                #positions.append(i)

    return len(positions), positions, crossing, avg, std